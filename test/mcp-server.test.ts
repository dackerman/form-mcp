...
describe('MCP Form Server', () => {
  ...
  it('should initialize and connect to the server', async () => {
    // The connection is established in beforeEach
    // Just verify we can get server info
    expect(client.serverInfo).toBeDefined();
    expect(client.serverInfo?.name).toBe('mcp-form-server');
  ...
  it('should handle concurrent form creation', async () => {
    ...
    const formIds = results.map(r => JSON.parse((r.content[0] as any).text).formId);
